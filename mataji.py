    ...
    if chat_history:
        for timestamp, conversation in chat_history.items():
            ...
            if st.button(f"Remove Conversation {timestamp}"):
                ...
                # Stop processing further buttons
                break
            st.write("---")
    else:
        st.write("No chat history available.")
        db.child(user_id).child("ChatHistory").remove()
...
def display_project_content(user_id):
    st.markdown("<p style='text-transform: uppercase; font-size: 20px; display: inline-block;'><img src='https://images.emojiterra.com/google/noto-emoji/unicode-15/animated/1f916.gif' style='vertical-align: middle; width: 30px;'> INTERACTIVE IMAGE CHAT PLATFORM </p>", unsafe_allow_html=True)
...
if choice == 'Sign up':
    ...
    if login:
        ...
        # Clear chat history when a new user logs in
        db.child(user_id).child("ChatHistory").remove()
        # Define the display_content function here
